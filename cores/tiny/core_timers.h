...
/*=============================================================================
  Veneer for the two ATtiny43U timers
=============================================================================*/
...
__attribute__((always_inline)) static inline void Timer1_SetToPowerup( void )
{
  ...
  // ICR1 = 0;
  // Disable all Timer1 interrupts
  TIMSK1 &= ~MASK3(TOIE1,OCIE1A,OCIE1B);
  // Clear the Timer1 interrupt flags
  TIFR1 |= MASK3(TOV1,OCF1A,OCF1B);
}
...
typedef enum
{
  ...
  Timer1_Prescale_Value_256    = B100,
  Timer1_Prescale_Value_1024   = B101,
  Timer1_T1_Falling            = B110,
  Timer1_T1_Rising             = B111,
  Timer1_Prescale_Index_1      = B001,
  Timer1_Prescale_Index_2      = B010,
  Timer1_Prescale_Index_3      = B011,
  Timer1_Prescale_Index_4      = B100,
...
{
  Timer1_Normal                     = B000,
  Timer1_Phase_Correct_PWM_FF       = B001,
  Timer1_CTC_OCR                    = B010,
  Timer1_Fast_PWM_FF                = B011,
  Timer1_Reserved_1                 = B100,
  Timer1_Phase_Correct_PWM_OCR      = B101,
  Timer1_Reserved_2                 = B110,
  Timer1_Fast_PWM_OCR               = B111
}
timer1_wgm_t;

__attribute__((always_inline)) static inline void Timer1_SetWaveformGenerationMode( timer1_wgm_t wgm )
{
  TCCR1A = (TCCR1A & ~MASK2(WGM11,WGM10)) | (((wgm & B011) >> 0) << WGM10);
  TCCR1B = (TCCR1B & ~MASK1(WGM12))       | (((wgm & B100) >> 2) << WGM12);
}
